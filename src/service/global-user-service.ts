...
class GlobalUserService {
  ...
  public currentGlobalUser: ComputedRef<Nullable<BaseGlobalUserFragment>> = computed(() => {
    return useFragment(this.baseUserFragment, this.currentUserQuery.result.value?.currentUser) || null
  })

  public currentImsUserData: ComputedRef<Nullable<BaseImsUserFragment>> = computed(() => {
    return useFragment(this.imsUserFragment, this.currentImsUserDataQuery.result.value?.currentImsUser) || null
  })
  ...
  private imsUserFragment = graphql(`
