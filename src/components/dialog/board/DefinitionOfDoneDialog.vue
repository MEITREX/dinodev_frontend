...
type DefinitionOfDoneTimeLeave = Omit<DefinitionOfDoneItem, 'implies'>
type DefinitionOfDoneTimeParent = Omit<DefinitionOfDoneItem, 'implies'> & { implies: DefinitionOfDoneTimeLeave[] }

const { project } = useProjectService()
const definitionOfDoneItems = computed(() => project.value?.projectSettings.definitionOfDone || [])

type DodConfirmState = DefinitionOfDoneConfirmState & { dodItem: DefinitionOfDoneItem, children: DodConfirmState[] }

const definitionOfDoneConfirmStates = ref(
  definitionOfDoneItems.value.map(item => initDodConfirmState(item)))

function initDodConfirmState(item: DefinitionOfDoneTimeParent): DodConfirmState {
  return {
    dodText: item.text,
    checked: false,
    reasonIfNotChecked: '',
    dodItem: item as DefinitionOfDoneItem,
    children: item.implies.map(child => (initDodConfirmState(child as DefinitionOfDoneTimeParent)))
  }
}
