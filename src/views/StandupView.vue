...
import TheEventList from '@/components/event/EventList.vue'
...
steps.value = shuffle(members.value.filter(m => m.online).map(m => m.name))
        ...
        <v-stepper :items="steps" v-model="e1">
          <template #item="{ raw }">
            <v-card>
              ...
              <v-card-subtitle>{{ raw }}'s activity this sprint: </v-card-subtitle>
              <v-card-text>
                <the-event-list :custom-member="raw" />
              </v-card-text>
            </v-card>
          </template>
        </v-stepper>
